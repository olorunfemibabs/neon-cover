...
const Sidebar = () => {
  ...
  return (
    <div
      ...
      ${
        toggleCollapse ? "w-20" : "w-1/5"
      } transition-[width] duration-300 ease-in-out `}
    >
      <div className="flex flex-col">
        <div className="flex items-center justify-between relative  px-4 border-b-[1px] border-[#C6C6C6]">
          <div
            className={`flex items-center justify-between gap-6 pt-[42px] pb-[45px] ${
              toggleCollapse && "pb-[49px]"
            }`}
          >
            <div className="">
              <ImShield
                className={`text-[#FFFFFF] text-[40px] ${
                  toggleCollapse && "text-[24px]"
                }`}
              />
            </div>
            <span
              className={` text-[30px] font-bold text-[#FFFFFF] ${
          ...
          {isCollapsible && (
            <button
              className={` absolute right-4 ${
                toggleCollapse && "relative rotate-180 left-2 mb-[26px]"
              }`}
              onClick={handleSidebarToggle}
            >
              <BsFillArrowLeftCircleFill
                color="#FFFFFF"
                className={`text-1.5rem ${toggleCollapse && "text-[16px]"}`}
              />
            </button>
          )}
        </div>
